...
class TableWidget extends Component {
    ...
    //设置序号
    setRoWIndex = () => {
        this.props.tableConfig.columns &&
            this.props.tableConfig.columns.forEach((v, i) => {
                if (v.title === '序号')
    ...
    render() {
        let { totalCount, data, loading, selectedRowKeys } = this.state;
        ...
        this.setRoWIndex(); //对数组数据进行设置
        let tableConfig = {
            columns, //表头
            rowSelection: showSelection ? rowSelection : showSelection, //勾选设置
            rowKey,
        };
        tableConfig = {
            loading,
            pagination: totalCount //分页设置
                ? {
                      size: 'small',
                      total: 10, //数据总数
                      pageSize: 5, //每页条数
